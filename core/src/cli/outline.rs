...
//! relative heading levels, and each entry carries `title`, `page` and `link`
//! attributes. `page` is the 1-based physical page number counting the cover and TOC.
//!
//! ```xml
...
pub fn build_outline_xml(headings: &[OutlineHeading]) -> String {
    ...
    // Stack the levels of the currently open `<item>` elements. Same nesting algorithm
    // as `cli::toc::write_entries` (a jump in level counts as a single step deeper).
...
/// Escape an XML attribute value. Since attributes are wrapped in `"`, `"` is escaped too.
...
mod tests {
    ...
    fn an_item_carries_title_page_and_link() {
        ...
        assert!(
            // Since it contains `"#` (the link attribute), the raw string is wrapped in `r##`.
    ...
    fn deeper_levels_nest_inside_the_parent_item() {
        ...
        // A-1 comes before A's closing tag (i.e. nested inside), and B comes after A is closed.
    ...
    fn a_level_jump_counts_as_one_nesting_step() {
        // An h1 -> h3 jump only goes one level deeper and keeps the tags balanced.
